...
class PerformanceTracker:
    ...
    def save_recommendation(self, rec: Dict) -> int:
        ...
        None değerleri güvenli şekilde işler.
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        if entry_price is None:
            print(f"  ⚠️  {rec.get('ticker', 'N/A')}: Fiyat bulunamadı, kayıt atlanıyor")
            conn.close()
            return 0
        ...
        try:
            ...
            print(f"  ⚠️  {rec.get('ticker', 'N/A')}: Geçersiz fiyat, kayıt atlanıyor")
            conn.close()
            return 0
        ...
        """, (
            ...
            rec.get("final_score") or rec.get("score", 0),
            rec.get("rating", "N/A"),
            rec.get("sector", "N/A"),
            rec.get("support"),
            rec.get("resistance"),
    ...
    def check_performance(self, days_to_check: List[int] = [7, 14, 30]):
        ...
        for days in days_to_check:
            ...
            for rec_id, ticker, entry_price, support, resistance in recommendations:
                ...
                # Performans daha önce hesaplandı mı?
                cursor.execute("""
